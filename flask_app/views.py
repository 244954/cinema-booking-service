...
from transactions.Booking import get_showing_detail, get_showings, select_seats_post, tickets_put, test_post, get_bookings_from_email, delete_booking_by_id, confirm_booking, cancel_booking
...
def offer_showings():
    if request.method == 'POST':
        return showings_post(dao_factory, request, True)
    else:
        return generate_response('HTTP method {} is not supported'.format(request.method), Status_code_not_found)


@app.route('/offer/showing', methods=['POST'])
def offer_showing():
    if request.method == 'POST':
        return showings_post(dao_factory, request, False)
    else:
        return generate_response('HTTP method {} is not supported'.format(request.method), Status_code_not_found)
...
@app.route('/bookings/confirm/<payment_id>', methods=['PUT'])
def confirm_book(payment_id):
    if request.method == 'PUT':
        return confirm_booking(dao_factory, payment_id, rabbit_channels.channel_publisher)
    else:
        return generate_response('HTTP method {} is not supported'.format(request.method), Status_code_not_found)


@app.route('/bookings/cancel/<payment_id>', methods=['DELETE'])
def cancel_book(payment_id):
    if request.method == 'DELETE':
        return cancel_booking(dao_factory, payment_id, rabbit_channels.channel_publisher)
    else:
        return generate_response('HTTP method {} is not supported'.format(request.method), Status_code_not_found)
